...
## ⚠️ Local Build Notes ⚠️

This fork was successfully built on Windows 10 with Visual Studio 2026 Professional using the VS 2022-compatible v143 toolset. The working configuration used MSVC `14.41.34120`, Windows SDK `10.0.26100.0`, `PlatformToolset=v143`, `VCToolsVersion=14.41.34120`, MSYS2/MinGW from `C:\MSYS64`, Git for Windows, NASM/YASM from MSYS2, Python `3.13.7` with the `polib` package installed, and Inno Setup 6 for installer generation. LAVFilters were built separately through the bundled build scripts with the same v143 toolset configuration.
